...
#line 23 "src\\tcpip\\tcpip.c"
#line 24 "src\\tcpip\\tcpip.c"
#line 25 "src\\tcpip\\tcpip.c"
#line 1 ".\\inc\\uart.h"
...
void dhcp_check (void)
{
	...
	if (mem_test (&localm[0]. IpAdr, 0, 4) == 0  && !(dhcp_tout & 0x80000000)) 
	{
		dhcp_tout = 0;
		tcp_state.bit.dhcp = 1;
		printf("\r[IP (DHCP): %d.%d.%d.%d]\r", localm[0]. IpAdr[0], localm[0]. IpAdr[1], localm[0]. IpAdr[2], localm[0]. IpAdr[3]);
		printf("[MASK: %d.%d.%d.%d]\r", localm[0]. NetMask[0], localm[0]. NetMask[1], localm[0]. NetMask[2], localm[0]. NetMask[3]);
		printf("[GATEWAY: %d.%d.%d.%d]\r", localm[0]. DefGW[0], localm[0]. DefGW[1], localm[0]. DefGW[2], localm[0]. DefGW[3]);
		fflush ((& __stdout));
		return;
	}
	...
	if (dhcp_tout == 0x80000000)
	{
		...
		dhcp_tout = 0;
		tcp_state.bit.dhcp = 0;
		for(i=0;i<4;i++)
		{
			localm[0]. IpAdr[i] = ip[i];
			localm[0]. NetMask[i] = mask[i];
			localm[0]. DefGW[i] = gateway[i];
		}
		
		printf("\r[IP (STATIC): %d.%d.%d.%d]\r", localm[0]. IpAdr[0], localm[0]. IpAdr[1], localm[0]. IpAdr[2], localm[0]. IpAdr[3]);
		printf("[MASK: %d.%d.%d.%d]\r", localm[0]. NetMask[0], localm[0]. NetMask[1], localm[0]. NetMask[2], localm[0]. NetMask[3]);
		printf("[GATEWAY: %d.%d.%d.%d]\r", localm[0]. DefGW[0], localm[0]. DefGW[1], localm[0]. DefGW[2], localm[0]. DefGW[3]);
		fflush ((& __stdout));
		return;
...
void user_init (void)
{
	...
	for(i=0;i<4;i++)
	{
		tcp_client_soc[i] = tcp_get_socket(0x01, 0, 120, tcp_callback_server);	
		tcp_listen(tcp_client_soc[i],atoi(cfg.tcp.port_serv_loc));	 
	}
	...
	tcp_server_soc = tcp_get_socket(0x02, 0, 60000, tcp_callback_client);
	...
	udp_soc = udp_get_socket (0, 0x01 | 0x02,udp_callback); 
	...
	switch (event)
	{
		...
		case 3:
			...
			else
			{
				...
				last_event = event;
				tcp_state.bit.serverConnected = 0;
			}
			fflush((& __stdout));
			return (1);
		...
		case 1:
			...
			{	
				...
				last_event = event;
				tcp_state.bit.serverConnected = 1;
			}
			fflush((& __stdout));
			return (1);
		...
		case 5:
			...
			if(modo == 0x00)
			{
				...
				printf("\r*** Receiver packet TCP client socket(%u): %s",socket,buf_rx);		  
			}else
			{
				rcv = rcv_cmd = 0x04;
...
U16 tcp_callback_server (U8 socket, U8 event, U8 *ptr, U16 par)
{
	return (tcp_callback (socket, event, ptr, par, 0x00));
}
...
U16 tcp_callback_client (U8 socket, U8 event, U8 *ptr, U16 par)
{
	return (tcp_callback (socket, event, ptr, par, 0x01));
}
...
U16 udp_callback (U8 socket, U8 *remip, U16 port, U8 *buf, U16 len)
{
	...
	if(strstr((const char *)udp_buf,"motel_all") != 0)
	{
		sprintf(str,"%s : %u.%u.%u.%u : %02X.%02X.%02X.%02X.%02X.%02X\r\n\0",lhost_name,localm[0]. IpAdr[0],localm[0]. IpAdr[1],localm[0]. IpAdr[2],localm[0]. IpAdr[3],
	...
	if((strstr((const char *)udp_buf,"SET") != 0))
	{
		...
		{
			if((strstr((const char *)udp_buf,scene[udp_buf[0]]) != 0))
			{  
	...
	if(strstr((const char *)udp_buf,"PORT") != 0)
	{
		for(i=0;i<8;i++)
		...
		{
			j = j + fwrite_line(aux[0], "tcp.cfg\0", LINE_PORT_UDP);	 
		...
		{
			j = j + fwrite_line(aux[1], "tcp.cfg\0", LINE_PORT_SERV_LOC);	 
		...
		{
			   fwrite_line(aux[2], "tcp.cfg\0", LINE_PORT_SERV_REM);	 
		...
		{
			   fwrite_line(aux[3], "tcp.cfg\0", LINE_IP_SERV_REM);	 
		...
		{
			   fwrite_line(aux[4], "tcp.cfg\0", LINE_IP_APP); 	 
		...
		if(j)
		{
			...
			for(i=0;i<5000000;i++)
				for(j=0;j<5000000;j++);
			((LPC_WDT_TypeDef *) ((0x40000000UL) + 0x00000) )->WDTC = 0x3FFFF;	 
			((LPC_WDT_TypeDef *) ((0x40000000UL) + 0x00000) )->WDFEED = 0xAA; ((LPC_WDT_TypeDef *) ((0x40000000UL) + 0x00000) )->WDFEED = 0x55;;
		}

	}else
	if(strstr((const char *)udp_buf,"RST") != 0)
	{
...
void tcp_task(void)
{
	...
	if((tcp_timer%(1000/100)))
		return;


	
	if(!(tcp_timer%(5000/100))) 
	{
		...
		localm[0]. NetMask[0], localm[0]. NetMask[1], localm[0]. NetMask[2], localm[0]. NetMask[3],  localm[0]. DefGW[0], localm[0]. DefGW[1], localm[0]. DefGW[2], localm[0]. DefGW[3]);	
	...
	{ 
			...
			for(i=0;i<4;i++)
				tcpSend (buf_tx, strlen(buf_tx), tcp_client_soc[i], 0x01);
	...
	if(tcp_get_state (tcp_server_soc) != 10)
	{
		tcp_state.bit.serverConnected = 0; 
		inet_aton((U8*)cfg.tcp.ip_serv_rem,ip);
