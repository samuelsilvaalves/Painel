...
unsigned short ir[400];
unsigned short ir_cmp[400];
...
char save_ir(unsigned short *dado,short addr,short channel,int index_file)
{
	...
	if(index_file == 0)	 
	{
		if(addr <= IR_ID6)
			...
			strcat(StrIdIr,"INFRARED\\TV\\IR_");	
		else
		if(addr <= IR_ID18)
		{
			 
			 
			strcat(StrIdIr,"INFRARED\\HOME\\IR_");	
		}else
		{
			if(atoi(cfg.file.mode_func_ar)) 	 
			{
				...
				if(ir_state.bit.ArSwing)
					strcat(StrIdIr,"INFRARED\\AR\\SWON\\IR_");
				else
					strcat(StrIdIr,"INFRARED\\AR\\SWOFF\\IR_");

			}else
			{
				 
				strcat(StrIdIr,"INFRARED\\AR\\SWOUT\\IR_");
			}
		}

		sprintf(str_aux,"%u.TXT\0",addr);	 
		strcat(StrIdIr,str_aux);			 
	}else
	if(index_file == 1)	 
	{
   		sprintf(StrIdIr,"%s%u.TXT\0","CMDIR\\IR_\0",addr);
	}else
	if(index_file == 2)	 
	{
...
char read_ir(unsigned short addr, int index_file)
{
	...
	if(index_file == 0)	 
	{
		...
		sprintf(StrIdIr,"%s%u.TXT\0",StrIdIr,addr);		 
...
{	
	...
	for(i=0;i<2480;i++)
	{
		sprintf(StrIdIr,"%s%u.TXT\0","INFRARED\\IR_\0",i);
		fdelete(StrIdIr);
	}	 
}
...
{	 	
   	char StrIdIr[30];
	memset(StrIdIr,0,sizeof(StrIdIr));
	sprintf(StrIdIr,"%s%u.TXT\0","INFRARED\\IR_\0",addr);
	fdelete(StrIdIr);	
}
...
char learn_ir(unsigned short addr, unsigned char channel, int index_file)
{
	...
	while(1)
	{
		...
		if(timer1_counter * 4 >= 50000)	 
		{
			if(!(((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00060) )->FIOPIN & (1<<25)))((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00060) )->FIOSET |= (1<<25); else ((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00060) )->FIOCLR |= (1<<25);;;			 
