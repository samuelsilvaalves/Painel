...
U16 ftp_fwrite (void *file, U8 *buf, U16 len) {
	...
	/*Bloco para verificar se houve alteração tcp via FTP*/
	memset(str,0,sizeof(str));
	strcpy(str,FILE_TCP_CFG);
	...
	if(strstr(last_file_ftp,str))
		ftp_state.bit.UpdateTcp = __TRUE;
	

	/*Bloco para verificar se houve alteração das cenas via FTP*/
	memset(str,0,sizeof(str));
	strcpy(str,"SCENE");
	...
	if(strstr(last_file_ftp,str))
		ftp_state.bit.UpdateScene = __TRUE;

	/*Bloco para verificar se houve alteração das configurações via FTP*/
	memset(str,0,sizeof(str));
	strcpy(str,"CFG.CFG");
	...
	if(strstr(last_file_ftp,str))
		ftp_state.bit.UpdateFileCfg = __TRUE;
  	
	//printf("Open File FTP: %s\rWrite FTP\r",last_file_ftp);
	//fflush(stdout);
